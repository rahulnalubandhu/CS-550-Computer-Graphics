...
const char *WINDOWTITLE = { "Project_2 " };
...
//scroll wheel buttton
const int SCROLL_WHEEL_UP = 3;
const int SCROLL_WHEEL_DOWN = 4;

const float SCREEN_WHEEL_CLICK_FACTOR = 5.f;
...
Animate( )
{
    ...
    Time = (float)ms*5000 / (float)MS_IN_THE_ANIMATION_CYCLE*5;
...
Display( )
{
    ...
    if (WhichPerspective == INSIDE)
    {
        ...
        gluLookAt( 0., 1.2, 1.,     0., 5., 13.,      0.,1.,0. );
    ...
    /////////// blades/////////
    // draw the cesna
    glCallList(CesnaList);

    //draw the blade(top)
    glPushMatrix();
    // glTranslatef(13., 2.0, -0.5);
    // glTranslatef(13., 2.0, 0.);////// for positioning blades
    ...
    glDisable( GL_DEPTH_TEST );
    glColor3f( 0., 1., 1. );
    //    DoRasterString( 0., 1., 0., "Hi~I am here." );
    ...
    glLoadIdentity( );
    glColor3f( 1., 1., 1. );
    DoRasterString( 5., 5., 0., "Project_2" );
...
InitLists( )
{
    ...
    CesnaList = glGenLists(1);
    glNewList(CesnaList, GL_COMPILE);
    ...
    glColor3f(0., 0., 1. );
    glBegin( GL_TRIANGLES );
    glVertex2f(  PROPELLER_RADIUS*3,  PROPELLER_WIDTH/2. );
    glVertex2f(  0., 0. );
    glVertex2f(  PROPELLER_RADIUS*3, -PROPELLER_WIDTH/2. );
    ...
    BladeList_Front= glGenLists(1);
    glNewList(BladeList_Front, GL_COMPILE);
    glColor3f(1., 0., 1.);
    glBegin( GL_TRIANGLES );
    glVertex2f(  PROPELLER_RADIUS*5,  PROPELLER_WIDTH/2. );
    glVertex2f(  0., 0. );
    glVertex2f(  PROPELLER_RADIUS*5, -PROPELLER_WIDTH/2. );
...
// The lighting of the cesna surfaces
