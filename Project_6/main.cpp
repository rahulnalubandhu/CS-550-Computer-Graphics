...
const char *WINDOWTITLE = "OpenGL/ Wadood Alam";
...
Display( )
{
	...
	if( WhichProjection == ORTHO )
		...
		gluPerspective( 100.f, 1.f,	0.1f, 1000.f );
	...
	//gluLookAt( 5.f, 5.f, 3.f,     0.f, 0.f, 0.f,     0.f, 1.f, 0.f );
	gluLookAt( 0.f, 0.f, 3.f,     0.f, 0.f, 0.f,     0.f, 1.f, 0.f );
		
	SetPointLight( GL_LIGHT0,  2., 4., 0.,  1., 1., 1. ); 	// white light
	...
	// Pattern->SetUniformVariable( "uColor", 0.7,0.4,0.1 );
	...
	if (ButtonControl != 3){
		Pattern->Use( );
		if (ButtonControl == 1 || ButtonControl == 4) {
            TimeVShader = Time;
        }
        if (ButtonControl == 1 || ButtonControl == 2) {
            TimeFShader = Time;
        }
		Ds = 0.4 * cos(M_PI*TimeFShader);
        Dt = 0.3 * sin(M_PI*TimeFShader);
        Ka = 0.1f;
        Kd = 0.6f;
        Ks = 0.3f; 
        ColorR = 0.858824;
        ...
        Pattern->SetUniformVariable("uBottonControl", ButtonControl);
		glCallList(SphereList);
		Pattern->Use(0);
