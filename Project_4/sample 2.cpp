...
const char *WINDOWTITLE = { "Lighting Animation -- Joe Graphics" };
...
Display( )
{
	...
	glPushMatrix( );
		SetPointLight( GL_LIGHT0,  2., 4., 0.,  1., 1., 1. );
		if( Light0On )
			glEnable( GL_LIGHT0 );
		else
			glDisable( GL_LIGHT0 );
	...
	glPushMatrix( );
		SetSpotLight( GL_LIGHT1,  -2., 3., -2.,  0., 0., -1.,  1., 0., 0. );
		if( Light1On )
			glEnable( GL_LIGHT1 );
		else
			glDisable( GL_LIGHT1 );
	...
	glPushMatrix( ); // blue
		SetMaterial( 1., 0., 0.,  8. );
		glTranslatef( 0., 0.,   8.*sin(2.*M_PI*Time) );
		SetPointLight( GL_LIGHT2,  0., 0., -0.50f,  0., 0., 1. );
		if( Light2On )
			glEnable( GL_LIGHT2 );
		else
			glDisable( GL_LIGHT2 );
