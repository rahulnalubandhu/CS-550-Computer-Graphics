...
Display( )
{
    ...
    if( AxesOn != 0 )
    {
    	...
    	glPushMatrix( );
		SetPointLight( GL_LIGHT0,  0., -2., 13.,  1., 1., 1. );
		if( LIGHT0On )
			glEnable( GL_LIGHT0 );
		else
			glDisable( GL_LIGHT0 );
    ...
    // glutSolidTorus(2.,5.,100, 60);
    // glDisable( GL_LIGHTING );
    
    //blue light
    
    SetPointLight(GL_LIGHT1, 0., 0, -0.5, 0., 0., 1.);
    glLightfv(GL_LIGHT1, GL_POSITION, Array3(19.8 * cos((Time* -360) * M_PI )/ 180, 0., 19.8 * cos((Time* -360) * M_PI / 180)));
	...
	glPushMatrix( );
		SetSpotLight( GL_LIGHT2,  4., 2.5, 2.5,  0., 0., -1.,  1., 0., 0. );
		if( LIGHT2On )
			glEnable( GL_LIGHT2 );
		else
			glDisable( GL_LIGHT2 );
    ...
    if (TexOn == OFF) {
        ...
        if (ImgOn == ONE) {
            ...
            Textures =
            BmpToTexture("/Users/nalubanr/Documents/CS550/Assignment3/SampleMac 2/worldtex.bmp", &ear_width, &ear_height);
            ...
            Textures =
            BmpToTexture("/Users/nalubanr/Documents/CS550/Assignment3/SampleMac 2/worldtex2.bmp", &ear_width, &ear_height);
            glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
            glGenTextures( 1, &tex0 ); // assign binding “handles”
            glBindTexture(GL_TEXTURE_2D, tex0);
...
//
    ...
    // DoRasterString( 0., 1., 0., "Project_4" );
